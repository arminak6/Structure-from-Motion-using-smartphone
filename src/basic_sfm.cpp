...
void BasicSfM::readFromFile(const std::string &filename, bool load_initial_guess, bool load_colors)
{
  ...
  // This wil die horribly on invalid files. Them's t3/he breaks.
...
void BasicSfM::solve()
{
  ...
  while (!seed_found)
  {
    ...
    std::cout << "Checking Seed ( Cam[" << ref_cam_pose_idx << "], Cam[" << new_cam_pose_idx << "])" << std::endl;
    std::cout << "Correspondences : " << max_corr << std::endl;
    std::cout << "n_inliers_H : " << n_inliers_H << std::endl;
    std::cout << "n_inliers_E : " << n_inliers_E << std::endl;
...
{        std::cout << "Sideward Motion" << std::endl;
  ...
  for (int iter = 1; iter < num_cam_poses_ - 1; iter++)
  {
    ...
    int n_new_pts = 0;
    std::vector<cv::Point2d> points0(0), points1(0);
    cv::Mat_<double> proj_mat0(3, 4), proj_mat1(3, 4), hpoints4D(3, 4);
      ...
      if (cam_pose_optim_iter_[cam_idx] > 0)
      {
        for (auto const &co_iter : cam_observation[cam_idx])
        {
              ...
              cam_observation[new_cam_pose_idx].find(pt_idx) != cam_observation[new_cam_pose_idx].end())
          {
            ...
            cv::Point2d point0(observations_[2 * cam_observation[new_cam_pose_idx][pt_idx]],
                               observations_[2 * cam_observation[new_cam_pose_idx][pt_idx] + 1]),
                point1(observations_[2 * cam_observation[cam_idx][pt_idx]],
                       observations_[2 * cam_observation[cam_idx][pt_idx] + 1]);
